...
async function handleLeaderboard(url, chain) {
  ...
  return jsonResponse({ chain, limit, updatedAt: Date.now(), rows });
}
...
async function handleMids(chain, bypassCache) {
...
async function handleApi(req, url, chain) {
  if (req.method !== "GET") {
    return null;
  }
  ...
  if (url.pathname === "/api/leaderboard") {
    try {
      return await handleLeaderboard(url, chain);
    } catch (error) {
      return jsonResponse({ error: String(error) }, 502);
    }
  }
...
export async function onRequest(context) {
  ...
  const chain = env?.HL_CHAIN ?? DEFAULT_CHAIN;
  const localApiResponse = await handleApi(request, url, chain);
  if (localApiResponse) return localApiResponse;

  return proxyToRemote(
    request,
    `${REMOTE_ORIGIN}${url.pathname}${url.search}`,
  );
}
