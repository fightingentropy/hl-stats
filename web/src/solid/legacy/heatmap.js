export function mountHeatmapPage() {
const heatmapState = {
  rows: [],
  metric: "change1d",
  search: "",
...
async function loadHeatmapData() {
  try {
    const payload = await fetchJson("/api/market/overview");
    const snapshot = payload?.snapshot?.symbols ? payload.snapshot.symbols : {};
    const relative = payload?.relativeStrength?.symbols
      ? payload.relativeStrength.symbols
      : {};
    const binance = payload?.binance24h?.symbols ? payload.binance24h.symbols : {};
...
function initHeatmap() {
  initHeatmapControls();
  const cleanupResize = initHeatmapResize();
  loadHeatmapData();
  const intervalId = window.setInterval(loadHeatmapData, 10_000);
